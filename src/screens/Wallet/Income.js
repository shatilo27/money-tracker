...
class Income extends Component {
  ...
  render() {
    const types = [
      ...
      {label: 'Дивиденды', value: 3},
      {label: 'Кредит', value: 4},
      {label: 'Разное', value: 5},
    ];
        ...
        <View style={styles.view}>
          <Background style={{flex: 1}}>
            <ScrollView style={{
              ...
              {this.state.wallet.data.income.length
                  ? this.state.wallet.data.income.map((item, key) => (
                      <List.Item
                          title={'+ ' + item.number + ' ' +
                  ...
                  : (
                      <View style={styles.notFoundWrapper}>
                        ...
                        <Text style={styles.notFoundText}>На данный момент у вас
                          нет доходов в данном кошельке</Text>
                      </View>
                  )}
            </ScrollView>
          </Background>
          ...
          <Portal>
            <Dialog visible={this.state.openModal}
              ...
              <Dialog.Title>Добавление доходов</Dialog.Title>
              <Dialog.Content>
                <TextInput
                    label="Сумма"
                    mode="outlined"
...
const mapDispatchToProps = dispatch => ({
  addIncome: (walletId, number, incomeType) => dispatch({
    type: 'ADD_INCOME',
    walletId: walletId,
    number: number,
